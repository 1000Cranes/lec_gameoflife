Meteor.appFunctions = {
  ...
  aliveNeighbors : function (cellID)
  {
    ...
    console.log('Checking Alive neighbors for - ' + cellID);  
    var im1jm1 = Cells.findOne({cell: im1 + ':' + jm1});
    var ijm1 = Cells.findOne({cell: i + ':' + jm1});
   
    if(im1 > -1 && jm1 > -1 && im1jm1 !== undefined && im1jm1.alive)
    {
      list.push(im1 + ':' + jm1);
      var updateArray = im1jm1.aliveNeighbors;
      updateArray.push(cellID);
      Cells.update({cell: im1 + ':' + jm1},{$set:{aliveNeighbors: updateArray}});
      //collection.update({A: true, B: true}, {$set: {C:false}});
    }

    if(i > -1 && jm1 > -1 && ijm1 !== undefined && ijm1.alive)
    {
      list.push(i + ':' + jm1);
      ijm1.aliveNeighbors.push(cellID);
    }
